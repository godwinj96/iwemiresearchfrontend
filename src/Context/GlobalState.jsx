...
export const GlobalStateProvider = ({ children }) => {
  ...
  const handleLogin = async (loginForm) => {
    try {
      const response = await fetch('http://localhost:8014/api/auth/login/', {
        method: 'POST',
        body: loginForm,
      });
  ...
  const checkSession = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8014/api/auth/profile/', {
        method: 'GET',
        headers: {
  ...
  const refreshAccessToken = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:8014/api/auth/token/refresh/', {
  ...
  const tokenVerify = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8014/api/auth/token/verify/', {
        method: 'POST',
